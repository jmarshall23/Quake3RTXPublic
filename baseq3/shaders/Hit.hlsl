...
float spot_attenuation(float r, float f, float d, float3 normal, float3 dir, float3 lightNormal, float3 lightOrigin, float3 origin) {
	float linearScale = 1.0 / 8000;
	float lightPointScale				=  7500;
	
	float distByNormal = -dot( dir, -lightNormal );
	if ( distByNormal < 0 ) {
		return 0;
	}
	float3 pointAtDist = lightOrigin + lightNormal * distByNormal;
	float radiusAtDist = ((r + 16) / d) * distByNormal;
		
	float3 distToSample = origin - pointAtDist;
	float sampleRadius = length(distToSample);
	...
	return radiusAtDist * ((photons / (d * d) * angle)  / 16); //photons / (d * d) * angle * radiusAtDist;
}
...
[shader("closesthit")] void ClosestHit(inout HitInfo payload,
  ...
  {    
	...
	{	  
		...
		if(length(lightInfo[i].light_normal) > 0)
		{
			falloff = spot_attenuation(lightInfo[i].origin_radius.w, 1.0, lightDistance, normal, normalize(centerLightDir), lightInfo[i].light_normal, lightInfo[i].origin_radius.xyz, worldOrigin);  
		}
		else {
			falloff = point_attenuation(lightInfo[i].origin_radius.w, 1.0, lightDistance, normal, normalize(centerLightDir));  
		}
		...
		if(falloff > 0.04)
		{
				...
				{
					ndotl += float3(lightInfo[i].light_color.x, lightInfo[i].light_color.y, lightInfo[i].light_color.z) * falloff; // normalize(centerLightDir); //max(0.f, dot(normal, normalize(centerLightDir))); 
				}
		}	  
		//  debug = normal;
	}
