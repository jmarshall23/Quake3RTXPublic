...
int			c_subdivisions;
int			c_gridVerts;
model_t* currentBrushModel = NULL;
...
=================
*/
static	void R_FlagSubmodels(lump_t* l) {
	dmodel_t* in;
	int			i, j, count;
	...
	for (i = 1; i < count; i++, in++) {
		for (int d = in->firstSurface; d < in->firstSurface + in->numSurfaces; d++) {
			s_worldData.surfaces[d].bmodelindex = i + 1;
		}
	}
}
...
void RE_LoadWorldMap( const char *name ) {
	...
	R_FlagSubmodels (&header->lumps[LUMP_MODELS]);
